...
def save_result(name, y_true, y_pred):
    ...
    save = pd.DataFrame({'Name':name, 'Precision':"{:.2f}".format(precision_score(y_true, y_pred)), 'Recall':"{:.2f}".format(recall_score(y_true, y_pred)), 'F1':"{:.2f}".format(f1_score(y_true, y_pred))}, index = [0])

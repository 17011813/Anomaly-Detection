...
class MLP_VAE:
    ...
    def _encoder(self):
        with tf.variable_scope('encoder',reuse=tf.AUTO_REUSE):
            ...
            z = tf.layers.dense(l2, self.lat_dim, activation=tf.nn.softmax)   # tf.nn.sigmoid 인기가요~?
        return z
    ...
    def _build_VAE(self):
        z = self._encoder()
        self.recons_X = self._decoder(z)
        
        with tf.variable_scope('loss'):
            self.all_loss = tf.reduce_sum(tf.square(self.input_X - self.recons_X), 1)          
            
        with tf.variable_scope('train'):            
            self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(self.all_loss)
    ...
    def test(self,input_data):
        ...
        for index in range(input_data.shape[0]):    # test 길이인 8957
            ...
            this_loss = self.sess.run(self.all_loss,feed_dict={ self.input_X: single_X })
...
for i in range(5):
    ...
    fine_test, real_test = test[:10], test[10:]    # 여기 10 만약에 batch size 10씩 아니면 꼭 바꿔야한다!!
...
save = pd.DataFrame({'Name':"Transfer-AE_{}".format(args.num), 'Precision':"{:.2f}".format(total[0]), 'Recall':"{:.2f}".format(total[1]), 'F1':"{:.2f}".format(total[2]), 'ACC':"{:.2f}".format(total[3]), 'AUC':"{:.2f}".format(total[4])}, index = [0])
